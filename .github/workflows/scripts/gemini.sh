...
for unsafe in .gemini .env GEMINI.md; do
  ...
  if [[ -e "$unsafe_path" || -L "$unsafe_path" ]]; then
    printf 'Pekin stopped: hosted editing will not load repository-controlled Gemini configuration: %s\n' "$unsafe" >&2
    exit 1
  fi
done
cd -- "$workspace"

export GEMINI_CLI_HOME="${RUNNER_TEMP:?}/pekin-gemini"

approval_mode=auto_edit
if [[ "${PEKIN_READ_ONLY:-0}" == 1 ]]; then
  approval_mode=plan
fi

arguments=(--skip-trust --approval-mode "$approval_mode" --admin-policy "$policy" --output-format json --prompt '')
if [[ -n "${PEKIN_MODEL:-}" ]]; then
  arguments+=(--model "$PEKIN_MODEL")
fi
