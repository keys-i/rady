...
Mention replies prefer an authenticated Codex when one is detected locally. The GitHub-hosted responder does not install Codex: it uses one cached runner job and calls the Gemini-first provider route directly. On public repositories, simple questions use free Gemini 3.5 Flash-Lite before Cerebras `gpt-oss-120b`; balanced and deep questions use Gemini 3.8 Flash, then Qwen 3.8 27B, then GPT-OSS. Add `RADY_GEMINI_API_KEY` as an Actions secret at repository or organisation scope. Optional `RADY_CEREBRAS_API_KEY` and `RADY_XAI_API_KEY` add Cerebras and paid Grok fallbacks when those accounts have billing access; Cerebras trial credits expire and are not a permanent free tier. Private and unknown repositories skip Gemini and Grok unless `RADY_GEMINI_PRIVATE_OK=true` or `RADY_XAI_PRIVATE_OK=true` explicitly enables that provider. Gemini's free tier may use content to improve Google products. xAI says API input and output are not used for training without permission, but normally retains them for 30 days unless Zero Data Retention is enabled for the xAI team. A local router is used instead of a separate third-party routing API. This configuration affects only read-only `@radyybot` replies, not native code, Dependasolve, or repair harnesses.
...
| Hosted mention replies | `RADY_GEMINI_PRIVATE_OK=true`, `RADY_XAI_PRIVATE_OK=true` for private-repository provider opt-in | `RADY_GEMINI_API_KEY`, optional `RADY_CEREBRAS_API_KEY`, optional `RADY_XAI_API_KEY` |
