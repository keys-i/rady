...
Use authenticated agent CLIs only on machines and runners you control. Reviews run on a trusted self-hosted runner, but public repositories reach it only after a GitHub-hosted preflight verifies an open, same-repository Dependabot pull request. Public reviews require the bounded Codex or Claude harness; arbitrary public pull requests and custom command adapters are rejected. Do not place a subscription credential, model API key, or harness login in a public workflow.
