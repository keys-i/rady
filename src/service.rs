...
fn service_cycle(
    ...
    for (index, token) in tokens.tokens()?.iter().enumerate() {
        let outcome = service_installation_cycle(arguments, token, mention_cursors);
...
fn service_installation_cycle(
    ...
    let mentions = mentions::sweep_with_token(arguments, token, mention_cursors);
...
pub(crate) fn targets(arguments: ServeArgs) -> Result<()> {
    ...
    let seed = env::var("RADY_TARGET_SEED")
        .ok()
        .filter(|value| !value.is_empty())
        .map(|value| value.parse::<usize>())
        .transpose()
        .map_err(|_| anyhow::anyhow!("RADY_TARGET_SEED must be a non-negative integer"))?
        .unwrap_or_default();
    let mut tokens =
