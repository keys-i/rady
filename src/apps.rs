...
pub fn require_app_identity(app: &Value, slug: &str) -> Result<()> {
    ...
    {
        bail!("the App credentials do not belong to the configured RadDuck App");
    }
...
mod tests {
    ...
    fn permissions_are_read_only_except_for_replies_and_reviews() -> Result<()> {
        ...
        app["permissions"]
            ...
            .unwrap()
            .remove("workflows");
        require_app_identity(&app, RADDUCK_SLUG)?;
        app["public"] = json!(false);
        assert!(require_app_identity(&app, RADDUCK_SLUG).is_err());
        assert!(validate_slug("RadDuck").is_err());
        Ok(())
    }
}
